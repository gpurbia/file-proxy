...
exports.upload = (req, res) => {
  ...
  if(!global.serviceConfig.FileLInk__Service__c !== config.services.SALESFORCE && !req.file && !req.body.fileData) {
    res.status(400).send('No file detected!');
  }
  else if(global.serviceConfig.FileLInk__Service__c == config.services.SALESFORCE && !req.file && !req.body.content_version) {
  ...
  */
  ...
  else if(req.file && global.serviceConfig.FileLInk__Service__c !== config.services.SALESFORCE) {
  ...
  */
  else if(global.serviceConfig.FileLInk__Service__c == config.services.AZURE && (req.body.fileData || req.body.fileUrl)) {
    azureAPI.upload(req, res);
  }
  else if(global.serviceConfig.FileLInk__Service__c == config.services.CLOUDINARY && (req.body.fileData || req.body.fileUrl)) {
    cloudinaryAPI.upload(req, res);
  }
  else if(global.serviceConfig.FileLInk__Service__c == config.services.AMAZONS3 && (req.body.fileData || req.body.fileUrl)) {
